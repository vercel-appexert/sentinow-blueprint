...
export async function getUserPermissions(
  ...
  try {
    ...
    // Get role-based permissions for project (if project context)
    let projectRolePermissions: any[] = [];
    if (projectId) {
      // Get user's project role (if they have one)
      ...
      if (projectMember) {
        const { data: projectRolePerms } = await supabase
          .from('role_permissions')
          .select(`
            permission:permissions(*)
          `)
        ...
        projectRolePermissions = projectRolePerms || [];
      }
    }
